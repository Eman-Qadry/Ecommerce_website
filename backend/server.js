...
function checkAdminUser() {
  ...
  pool.query('SELECT * FROM users WHERE email = $1', ['admin@bookstore.com'], (err, result) => {
    if (err) {
...
app.get('/api/users/profile', verifyToken, (req, res) => {
  db.get('SELECT id, email, name, role, created_at FROM users WHERE id = $1', [req.user.id], (err, user) => {
    if (err) {
      return res.status(500).json({ error: 'Database error' });
    }
    if (!user) {
...
app.put('/api/users/profile', verifyToken, (req, res) => {
  ...
  db.get('SELECT password FROM users WHERE id = $1', [req.user.id], (err, user) => {
    if (err) {
      return res.status(500).json({ error: 'Database error' });
    }
...
app.get('/api/products/:id', (req, res) => {
  db.get('SELECT * FROM products WHERE id = $1', [req.params.id], (err, product) => {
    if (err) {
      return res.status(500).json({ error: 'Database error' });
    }
    if (!product) {
